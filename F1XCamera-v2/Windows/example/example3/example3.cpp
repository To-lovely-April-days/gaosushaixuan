...
void Example()
{
	...
	std::cout << bandz->size() << std::endl;
	if (bandz->size() > 0)
	{
		(*bandz)[0]->setEnable(true);
		auto ofst = (*bandz)[0]->getOffset();
		ofst->setValue(ofst->getMin());
		(*bandz)[0]->setOffset(ofst);

		auto sz = (*bandz)[0]->getSize();
		sz->setValue(sz->getMin()+8);
		(*bandz)[0]->setSize(sz);
	}
	camera->SetBandROI((*bandz)[0]);
	std::cout << ("enable:") << (*bandz)[0]->getEnable() << std::endl;
	std::cout << ("offset:") << (*bandz)[0]->getOffset()->getValue() << std::endl;
	std::cout << ("size:") << (*bandz)[0]->getSize()->getValue() << std::endl;
	std::cout << ("max frame rate:") << camera->GetAcquisitionFrameRate()->getMax() << std::endl;

	std::cout << ("current exposure:") << camera->GetExposureTime()->getValue() << std::endl;
...
void UserGrabedDataEvent(GrabData* grabData)
{
	if (grabDatas.size() < 10)
	{
		...
		std::cout << "receive" << grabDatas.size() << " the Hyperspectral data" << std::endl;
	}
}
